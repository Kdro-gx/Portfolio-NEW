...
async function dataRoutes(fastify, options) {
  ...
  // Skill Graphs (Web Graphs)
  fastify.get("/getskillgraphs", dataController.getSkillGraphs);
  fastify.post(
    "/addskillgraph",
    { preHandler: [verifyJWT] },
    dataController.addSkillGraph
  );
  fastify.put(
    "/updateskillgraph/:id",
    { preHandler: [verifyJWT] },
    dataController.updateSkillGraph
  );
  fastify.delete(
    "/deleteskillgraph/:id",
    { preHandler: [verifyJWT] },
    dataController.deleteSkillGraph
  );
  ...
  // About Me Management
  fastify.get("/aboutme", dataController.getAboutMe);
  fastify.put(
    "/aboutme",
    { preHandler: [verifyJWT] },
    dataController.updateAboutMe
  );

  // Timeline Management
  fastify.get("/timeline", dataController.getTimeline);
  fastify.get("/timeline/years", dataController.getTimelineYears);
  fastify.post(
    "/timeline",
    { preHandler: [verifyJWT] },
    dataController.addTimeline
  );
  fastify.put(
    "/timeline/:id",
    { preHandler: [verifyJWT] },
    dataController.updateTimeline
  );
  fastify.delete(
    "/timeline/:id",
    { preHandler: [verifyJWT] },
    dataController.deleteTimeline
  );
  ...
  fastify.post(
    "/reorder",
    { preHandler: [verifyJWT] },
    dataController.reorderItems
  );

  // Settings Management (Resume URL and Social Links)
  fastify.get("/settings", dataController.getSettings);
  fastify.put(
    "/settings",
    { preHandler: [verifyJWT] },
    dataController.updateSettings
  );

  // Admin Management
  fastify.post("/initialize-admin", dataController.initializeAdmin);
  fastify.post("/reset-admin", dataController.resetAdmin);
  fastify.get("/debug-admin", dataController.debugAdmin);
  fastify.post(
    "/setAdminCredentials",
    { preHandler: [verifyJWT] },
  ...
  fastify.get("/top-langs", async (request, reply) => {
    ...
    const githubAPIUrl =
      "https://github-readme-stats.vercel.app/api/top-langs/?username=Kdro-gx&langs_count=8&layout=compact&theme=react&hide_border=true&bg_color=1F222E&title_color=F85D7F&icon_color=F8D866&hide=Jupyter%20Notebook,Roff";
    try {
